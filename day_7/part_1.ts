...
const main = async (programName: Test) => {
  ...
  const AmpA = new IntcodeCPU(program, [channelStart], channelAB);
  ...
  const AmpE = new IntcodeCPU(program, [channelDE], channelEnd);

  const phaseSettings = perms([0, 1, 2, 3, 4]);
  let best = {
  ...
  console.log(`Highest thruster value was ${best.value} for ${best.setting}`);
};
